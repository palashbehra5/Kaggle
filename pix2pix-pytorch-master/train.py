...
root_path = "dataset\\"
train_set = get_training_set(root_path, "dataset\\train")
test_set = get_test_set(root_path, "dataset\\test")
...
for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
    ...
    if(avg_psnr > max_psnr): 
        ...
        for batch in testing_data_loader:
            ...
            input, target = batch[0].to(device), batch[1].to(device)
            prediction = net_g(input)
            img = np.array(prediction.squeeze(0).permute(1,2,0).cpu().data)

            # Scaling to save image to disk
            img = (img*255).astype('uint8')

            io.imsave("dataset\\test\\prediction\\"+str(count)+".png", img)
            count+=1
